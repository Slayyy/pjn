...
from pywnxml import synset
from pywnxml import WNXMLParser
...
class WNQuery:
    ...
    def simLeaCho(self, wnid1, wnid2, pos):
        first = self.bfs_shortest(wnid1, wnid2, pos)
        second = self.bfs_shortest(wnid2, wnid1, pos)


        path_length = min(first or second, second or first) if first or second else None
        if path_length:
    ...
    def bfs_shortest(self, wnid1, wnid2, pos):
        import queue
